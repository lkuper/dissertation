...
parallel execution and schedule nondeterminism.  \lk{Maybe I don't
  ...
  ``organization'' bullet points below.} As I will show in
...
\TODO{Say something about generalizing from least-upper-bound writes
  to inflationary and commutative writes, and from threshold reads to
  generalized threshold reads and then to threshold functions.  Also
  say what we've proved about what!}
...
detail, in Chapter~\ref{ch:lvish}\lk{Refer to a specific section?}).
...
Chapter~\ref{ch:deterministic} I show how to extend CvRDTs to support
...
\begin{itemize}
  \item \emph{Lattice-based data structures}: In
    ...
    Appendices~\ref{app:fixme} and~\ref{app:fixme}\TODO{Freshen up the
      Redex models of $\lambdaLVar$ and $\lambdaLVish$ and add them as
      appendices.} contain runnable versions of $\lambdaLVar$ and
  ...
  \item \emph{general}: In Chapter~\ref{ch:lvars}, I defend the
    ...
    \TODO{Something about generalizing from least-upper-bound writes
      to inflationary and commutative writes, and from threshold reads
      to generalized threshold reads and then to threshold functions,
  ...
  \item \emph{provably deterministic}: In Chapter~\ref{ch:lvars}, I
    ...
    aforementioned @put@ and @get@ operations on LVars.\TODO{Also say
      something about the proof for generalized inflationary and
      commutative writes and for threshold functions.}
...
\lk{I was going to have something like, ``The material in this chapter
  is based on research done jointly with...'' in individual chapters,
  and cite the papers there.  However, the material in the chapters
  doesn't exactly correspond to the papers; for instance, the stuff
  about generalized threshold sets is from the DISC paper, but it's in
  chapter 2.  So I'm just going to try citing all the papers up
  front.}
