...
\emph{language-level} guarantee of determinism, ensuring that
...
\section{The deterministic-by-construction parallel programming landscape}

Much work has been done on deterministic-by-construction parallel
programming models over the years and decades.  Taking a moment to
survey the landscape, we find:

\begin{itemize}
\item \emph{No-shared-state parallel programming.}  One classic
...
\item \emph{Kahn process networks and data-flow programming.}  In
...
\item \emph{Single-assignment parallel programming with IVars.}  In
  ...
  \lk{I'm not sure if this next part about monad-par belongs
    here. Maybe it should be moved to, say, the LVish chapter when we
    talk about how LVish extends monad-par.} Although most of these
...
\item \emph{Programming with imperative disjoint parallelism.}
...
At first glance, the four programming models listed
above---no-shared-state parallelism, dataflow parallelism,
...
\lk{TODO: make this next chunk of text way less colloquial.  The idea
  here is to say that there are two problems.  One is that a big
  toolkit of unrelated choices is bad because what if we need to use
  more than one?  It is not clear how well they compose.  The second
  is that a big toolkit of unrelated choices is bad because it's more
  to learn and reason about.  We show that this is unnecessary because
  in fact they all fit into the same programming model.}

Moreover, it might be fine if we just had a big toolkit of unrelated
...
multiple parallelizable kernels that span different points in this
space?  For instance, what if we have an application that uses
data-flow pipeline parallelism but also does disjoint parallel
mutation of a store?
...
\lk{TODO: say something about monotonicity as a unifying principle.}
...
\section{Thesis statement, and organization of the rest of this disseration}\label{s:thesis}
...
\begin{itemize}
  \item \emph{Lattice-based data structures}: In Chapter~\ref{ch:lvars}, I formally define
    ...
    experimentation.\footnote{$\lambdaLVish$ is the \emph{LVish
      ...
      library discussed in Section~\ref{ss:lvish}.\lk{TODO: That's not quite right -- it's the deterministic fragment of what's in \cite{Freeze-paper}.  Figure out how to organize this.}}
  ...
  \item \emph{distributed programming}: In Chapter~\ref{ch:crdts}, I defend the claim that
    ...
    LVar-style threshold reads to conflict-free replicated data types. \lk{TODO: Add more detail here.}
\end{itemize}
