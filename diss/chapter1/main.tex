...
\TODO{Say something about Section~\ref{s:lvars-generalizing}, and if
  there's going to be a proof about the generalized model, mention
  that.}
...
\TODO{Edit this part.  It's really just two case studies: k-CFA and
  PhyBin.  I talk about the graph traversal thing earlier in the
  chapter.}
...
\item Third, I describe using LVish to parallelize
  ...
  effect specification system in LVish.  \TODO{Revise this; say
    something about Section~\ref{s:lvars-generalizing}.}
...
\begin{itemize}
  ...
  \item \emph{general}: In Chapter~\ref{ch:lvars}, I defend the
    ...
    is parameterized by the choice of lattice.  Hence $\lambdaLVar$ is actually a
    \emph{family} of calculi, varying by choice of lattice.
    \TODO{Something about Section~\ref{s:lvars-generalizing} should go
      here as well.}
  ...
  \item \emph{deterministic}: In Chapter~\ref{ch:lvars}, I defend the
    ...
    aforementioned @put@ and @get@ operations on LVars.  \TODO{If
      there's a proof to accompany the generalized model in
      Section~\ref{s:lvars-generalizing}, something about that should
      go here.}
  ...
  \item \emph{distributed programming}: In
    ...
    LVar-style threshold reads. \TODO{Say a little bit more about what
      the new contribution is wrt mixing consistency levels.}
\end{itemize}
