...
We implemented two versions of the $k$-CFA algorithm using data
structures that the LVish library provides.  One uses the LVish
library's reference implementation of a set, which uses a pure
@Data.Set@ wrapped in a mutable container.  The other is a lock-free
set based on concurrent skip lists~\cite{art}.\footnote{In fact, LVish
  is the first project to incorporate \emph{any} lock-free data
  structures in Haskell, which required solving some unique problems
  pertaining to Haskell's laziness and the GHC compiler's assumptions
  regarding referential transparency.\lk{Ryan, I don't want this
    statement to go unsupported---is there anything I can point to
    about this?  Even a blog post?}}

We evaluated both the reference and the lock-free $k$-CFA
...
for the benchmarks in this section.)  The blue line in
Figure~\ref{fig:bench} shows what a linear parallel speedup would look
like as cores are added, The yellow and green lines show the parallel
speedup of the ``blur'' benchmark for the reference implementation and
lock-free implementation of $k$-CFA, respectively, and the purple and
red lines show parallel speedup of the ``notChain'' benchmark for the
reference and lock-free implementations, respectively.

The results for the reference implementations are normalized to the
same baseline as the lock-free results for the single-core versions.
At one and two cores, the lock-free $k$-CFA implementation (shown in
green) is $38\%$ to $43\%$ slower than the reference implementation
(in yellow) on the ``blur'' benchmark.  The reference implementation,
however, stops scaling after four cores.  Even at four cores, variance
is high in the reference implementation (min/max $0.96$s / $1.71$s
over 7 runs).  Meanwhile, the lock-free implementation continues
scaling and achieves an $8.14\times$ speedup on twelve cores ($0.64$s
at $67\%$ GC productivity).
