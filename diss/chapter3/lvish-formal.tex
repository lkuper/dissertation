...
$lv$.  Eventually, the program is guaranteed to fault, raising
$\error$ by way of the {\sc E-Put-Err} rule, because $\lubp{p_1}{p_2}
= \topp$.  Before that happens, though, $\getexp{lv}{\setof{p_1,
    p_2}}$ could return either $p_1$ or $p_2$.  Therefore, @get@
\emph{can} behave nondeterministically---but this behavior is not
observable in the final outcome of the program.

\subsection{Freezing and the $\FAW$ primitive}\label{subsection:language-freezing}
