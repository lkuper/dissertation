...
Instead of limiting ourselves to a single data type of data structure,
then, I propose taking the more general notion of monotonic data
structures as the starting point for a new model for deterministic
parallelism.  The model I propose generalizes IVars to \emph{LVars},
thus named because the states an LVar can take on are elements of an
application-specific \emph{lattice}.\footnote{As I'll explain in
  Section~\ref{s:lvars-domains}, this ``lattice'' need only be a {\em
  ...
  greatest element''.}  In this chapter, I define LVars and use them
to define $\lambdaLVar$, a deterministic parallel calculus with shared
...
(Section~\ref{s:lvars-independence}), that would {\em not} hold in a
typical language with shared mutable state, but holds in our setting
because of the monotonic semantics of LVars.
