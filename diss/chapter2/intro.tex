...
%% variables), and a new parallel calculus to study it, $\lambdapar$.
%% $\lambdapar$ is a parallel calculus with shared state
...
%% data structures to leverage results from  $\lambdapar$.
...
%% structure can leverage our determinism result for $\lambdapar$. To
...
%% This paper focuses on the theory of $\lambdapar$, but along the way we will
...
%% \item Our main result is a proof of determinism of \lambdapar{}
...
%% \item We provide a practical implementation of the \lambdapar{} model as a
%%   generalization of the {\tt monad-par} library for Haskell (Section~\ref{section:motivation}).  Also we provide a
%%   Redex model, interpreters, and a data-race detector for [extended] \lambdapar{}
%%   (with limited nondeterminism) to support the study of \lambdapar{} programs.
