...
\rn{Perhaps it's not important, but we don't seem to specify whether
  $\myleq$ on the numeric components of the pair is an IVar style
...
For our first example of a $\lambdapar$ program, we choose the elements of our lattice to be
...
%   rule in Figure~\ref{f:lambdapar-getvalblock}.  The reason why we
%   don't have that in Figure~\ref{f:lambdapar-refl} is because the {\sc
