...
%% {The basic $\lambdaLVar$ model, as we've presented it, is useful for
...
%%   believe that $\lambdaLVar$ is also useful as a starting point for
%%   further extension and exploration, including limited forms of
%%   nondeterminism.  }

%% In this section we give an example of a natural extension of
%% $\lambdaLVar$ that provides additional capabilities.  We use as our
...
%% the $\BUMP$ after the final $\IN$, using the implicit
...
%% $\GET$/$\PUT$/$\BUMP$ that logs all effects to each location, along
...
%%   graphs induced by $\lambdaLVar$, there are not yet known methods for reducing the space
...
%% relation, if any $\BUMP$ operation is not strictly later in time than
%% a $\PUT$ or $\GET$ on the same location, then that $\BUMP$ is
...
%% shared, increment-only counters as a $\lambdaLVar$ domain
...
%% %
%% Doing so does not require an extension to $\lambdaLVar$, but it merits
...
With \termfont{unique} in place, we can write programs like the following, in which two
parallel
...
%%     \end{align*}
%%   \caption{\footnotesize Rewrite rules for desugaring $\lambdaLVar$ expressions with the \lstinline|unique| construct to plain $\lambdaLVar$ expressions without \lstinline|unique|.  Here we use ``$L$:'', ``$R$:'', and ``$J$:'' to
...
%%  %   {\url{http://github.com/rrnewton/lambdaLVar_interps}}.
%% }
%%   \label{f:desugar}
%% \end{figure}
...
%% In the context of
%% $\lambdaLVar$, there is an interplay between $\lambda$-calculus
