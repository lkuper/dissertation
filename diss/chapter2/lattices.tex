...
\includegraphics[width=5in]{chapter2/figures/ExampleLattices2.pdf} 
  \caption{Example lattices: (a) IVar containing a natural number; (b)
    ...
    by $\leq$ (see Section~\ref{subsection:lvars-fixme}\TODO{Add
      forward reference to section on generalized monotonic writes.}).
    Subfigure (b) is annotated with example threshold sets that would
    correspond to a blocking read of the first or second element of
    the pair (see
...
on, in Section~\ref{s:lvars-fixme}\TODO{Add forward reference to
  section on generalized threshold reads.}, I will explain how to
