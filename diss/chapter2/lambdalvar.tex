...
The context reduction relation $\ctxstepsto$ has only one rule, {\sc
...
context.\footnote{In fact, my motivation for splitting the operational
  semantics into a reduction semantics and a context relation is to
  isolate the nondeterminism of the context reduction relation and
  make it easier to reason about in the determinism proof of
...
\ie, if we want to first allocate a new LVar with @new@ and then call
@put@ on it.
