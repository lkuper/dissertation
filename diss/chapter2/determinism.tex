...
application-specific lattice $D$.)  Because $\pi$ is invertible, it is
a bijection, and from that fact it follows that if some location $l$
is not in the domain of some store $S$, then $\pi(l) \notin
...
\begin{proof}
  ...
  \dom{\pi(S)}$. \TODO{Say something about
...
run. \TODO{Get a citation for this.}  Although $\lambdaLVar$ is not
