...
\begin{lem}[Diamond]\label{lem:lvars-diamond}
  ...
  \leq 1$.  \TODO{Figure out if we need to mention permutations in the
...
\begin{cor}[Strong Local Confluence]\label{cor:lvars-strong-local-confluence}
  ...
  there exist $\conf_c, i, j$ such that $\conf' \parstepsto^i \conf_c$
  and $\conf'' \parstepsto^j \conf_c$ and $i \leq 1$ and $j \leq 1$.
  \TODO{If the statement of Lemma~\ref{lem:lvars-diamond} changes to
    mention permutations, then figure out how the statement of this
    one has to change as well.}
\end{cor}
\begin{proof}
...
\begin{thm}[Determinism]\label{thm:lvars-determinism}
  ...
  \conf''$.  \TODO{If the statement of Lemma~\ref{lem:lvars-diamond}
    changes to mention permutations, then figure out how the statement
    (and proof) of this one has to change as well.}
\end{thm}
\begin{proof}
