...
\begin{lem}[Generalized Independence]\label{lem:generalized-independence}
  ...
  \[ \config{\Ustore(S)}{e} \parstepsto \config{\Ustore(S')}{e'}, \]
  where $\Ustore$ is a store update operation meeting the following conditions:
  \begin{itemize}
    \item $\Ustore$ is non-conflicting with $\config{S}{e} \parstepsto \config{S'}{e'}$,
    \item $\Ustore(S') \neq \topS$, and
    \item $\Ustore$ is freeze-safe with $\config{S}{e} \parstepsto \config{S'}{e'}$.
  \end{itemize}
\end{lem}
}
...
\begin{lem}[Generalized Clash]\label{lem:generalized-clash}
  If $\config{S}{e} \parstepsto \config{S'}{e'}$ (where
  $\config{S'}{e'} \neq \textup{\error}$), then we have that:
  \[ \config{\Ustore(S)}{e} \parstepsto^i \textup{\error}, \]
  where $i \leq 1$ and where $\Ustore$ is a store update operation meeting
  the following conditions:
  \begin{itemize}
    \item $\Ustore$ is non-conflicting with $\config{S}{e} \parstepsto \config{S'}{e'}$,
    \item $\Ustore(S') = \topS$, and
    \item $\Ustore$ is freeze-safe with $\config{S}{e} \parstepsto \config{S'}{e'}$.
  \end{itemize}
\end{lem}
}
