...
\newcommand{\LemLatticeStructureConcise}{
\begin{lem}[Lattice structure]\label{lem:lattice-structure-concise}
If $(D, \userleq, \bot, \top)$ is a lattice then $\Freeze{D, \userleq,
  \bot, \top}$ is as well.
\end{lem}
}
...
\newcommand{\LemAlwaysCrash}{
\begin{lem}[Always Crash]\label{lem:always-crash}
Suppose $\config{S}{e} \parstepsto \error$. Then if $\config{S}{e}$ goes to a halting state,
then that state will be $\error$. 
\end{lem}
}
...
\newcommand{\LemClash}{
\begin{lem}[Clash]\label{lem:clash}
  If $\config{S}{e} \parstepsto \config{S'}{e'}$ (where
  $\config{S'}{e'} \neq \textup{\error}$), then:
  ...
  \lk{This lemma *may* not actually need the $\lubstore{S'}{S''}
    \statuseq S$ assumption.  Still to be determined.}
\end{lem}
}
