...
\begin{definition}\label{def:lvars-permutation}
  ...
  \begin{enumerate}
    ...
    \item is is the identity on all but finitely many elements of
      $Loc$.
  \end{enumerate}
\end{definition}
}
...
\begin{definition}\label{def:lvars-permutation-store}
  A permutation of a store $S$ is a function $\pi$ defined as
  follows:
  \begin{displaymath}
    \begin{array}{ll}
      \pi(\topS) = \topS \\
      \pi(\store{\storebindingRaw{l_1}{d_1}, \dots,
        \storebindingRaw{l_2}{d_2}}) &=
      \store{\storebindingRaw{\pi(l_1)}{d_1}, \dots,
        \storebindingRaw{\pi(l_2)}{d_2}} \\
    \end{array}
  \end{displaymath}
\end{definition}
}
...
\begin{definition}\label{def:lvars-permutation-configuration}
  ...
  defined as follows: $\pi(\config{S}{e}) = \config{\pi(S)}{\pi(e)}$.
\end{definition}
}
