...
\begin{definition}[store update operation]\label{def:store-update-operation}
  Given a set of state update operations $U_p$, a \emph{store update
    operation} is a function $U_S$ from stores to stores, defined as
  follows: $U_S(S)$ is a store such that $\dom{U_S(S)} = \dom{S}$ and,
  for each $l \in \dom{S}$, $(U_S(S))(l) = u_{p_i}(S(l))$, where
  $u_{p_i} \in U_p$.
\end{definition}
}
