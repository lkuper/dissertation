...
parallelism.  Then, in Section~\ref{crdts}, I discuss my ongoing work
on the relationship between LVars and \emph{conflict-free replicated
  data types} for eventually consistent distributed systems, and what
I plan to work on next.  For brevity, I do not discuss my previous
...
tasks.  Shared state, if not disallowed entirely, is restricted to a
...
\emph{lattice}.  LVars ensure determinism by allowing only
monotonically increasing writes and ``threshold'' reads that block
until a lower bound in the lattice is reached.  Together, monotonic
writes and threshold reads yield a provably deterministic parallel
programming model: programs will have the same observable result in
spite of parallel execution and schedule nondeterminism.  Our
determinism result is \emph{lattice-generic}: threads can communicate
through \emph{any} shared data structure, so long as the states the
data structure can take on can be viewed as elements of a lattice and
updates are monotonically increasing.  This generality allows a
...
the model with a lattice with one ``empty'' state and multiple
``full'' states (where $\forall{i}.\; \mathit{empty} <
