...
\lk{The paper is hard to read because they don't actually define a
  ...
  is ``CvRDT''.  So I guess that that means that a CvRDT satisfies the
...
\item Extend the definition of CvRDTs to add a mechanism for
  ...
  consistent read property for extended CvRDTs.  In practice, though,
  a combination of threshold reads and regular reads may make the most
  sense.
\item Extend the LVars model to allow non-lub update operations---that
  ...
  $\lambdaLVish$ syntax, and add the following new rule {\sc E-Bump}
  to the operational semantics, and prove determinism for the
  resulting extended language:
\begin{mathpar}
