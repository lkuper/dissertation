...
\begin{itemize}
\item Add a mechanism for specifying threshold reads to CvRDT
  specifications---say, by adding a $t$ operation to the
...
\item Extend the LVars model to allow non-lub update operations, that
  is, update operations other than @put@.  In fact, the @bump@
  operation discussed in Section~\ref{ss:lvish}---which is already
  part of the LVish library, but not yet formalized in
  $\lambdaLVish$---is one such non-lub update operation.
\end{itemize}
