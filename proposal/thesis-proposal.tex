...
\emph{strong convergence}, meaning that all correct replicas to which
the same updates have been delivered have equivalent state.  CRDTs can
be specified in two ``styles'': \emph{state-based}, also known as
...
I focus on the state-based style here because it is closer to the
LVars model, although state-based specifications can emulate
operation-based ones, and vice versa.\lk{Explain this better.}
...
\item The merge method $m$ computes the least upper bound of the two
  states.
\item State is monotonically non-decreasing across updates: if $u$
  updates a state $s$ to $s'$, then $s \leq s'$.
\end{itemize}
As we can see from this definition, CvRDTs have much in common with
LVars; but they also have the following differences:
\begin{itemize}
