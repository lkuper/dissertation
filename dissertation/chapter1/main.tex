...
\emph{Deterministic-by-construction} parallel programming models, though, offer
the promise of freedom from subtle, hard-to-reproduce nondeterministic bugs in
parallel code.  While there are many ways to construct deterministic parallel
programs and to verify the determinism of individual programs,
deterministic-by-construction programming models are those that provide a
\emph{language-level}, static guarantee of determinism.
% : deterministic programs are the only programs that can be expressed within the model.
...
states an LVar can take on are elements of an specific
...
\item The @put@ operation can only make the state of an LVar ``grow''
  ...
  For example, on LVars of collection type, such as sets, @put@ typically inserts an element.
...
are compatible with LVars as well.  Furthermore, as \either{I}{we} show in
...
formally extend the model to support such arbitrary update operations
...
\item Second, \either{I}{we} add the ability to attach \emph{event handlers} to an
...
LVars.  \either{I will describe}{In Section~\ref{ch:lvish}, we describe} the proof obligations for data structure
...
introducing the above features with examples, \either{Chapter}{Section}~\ref{ch:lvish}
...
\item First, \either{I}{we} describe using LVish to parallelize a control flow
  ...
  parallelized.  \either{I}{We} contrast the LVish implementation with the original
...
\item Second, \either{I}{we} describe using LVish to parallelize
  ...
  the PhyBin application crucially relies on
...
This paper revises and expands our previously published papers on
LVars~\cite{Freeze-paper, LVars-paper} and LVish~\cite{effectzoo}.
The material in Section~\ref{s:lvars-generalizing} is appearing for
the first time.  The $\lambdaLVar$ calculus of Section~\ref{ch:lvars}
...
but generalizes it to allow arbitrary update operations, as described
above.  Additionally, Section~\ref{ch:lvish} describes the most recent
version of the LVish Haskell library, which has been updated to run
against the most recent version\footnote{GHC 7.8.3, at the time of
