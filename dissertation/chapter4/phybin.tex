...
Listing~\ref{listing-hashrf-alg} gives psuedocode for the HashRF
algorithm.  The second phase of the algorithm is still $O(N^2)$, but
it only needs to read from the much smaller @tree_set@ during this
phase.  All loops in Listing~\ref{listing-hashrf-alg} are potentially
parallel.

\TODO{This is ugly -- shouldn't we be writing this in some nicer
  pseudocode syntax?}

\singlespacing
\begin{lstlisting}[float, caption={Pseudocode of the HashRF algorithm for computing a tree
    ...
    exactly one of the two trees being compared.}, label=listing-hashrf-alg]
