...
\begin{algorithm}
  \begin{algorithmic}[0] % 0 means no line numbering
    ...
    \hspace{3em}\LineComment{Add $t$ to set of trees pointed at by $\mathit{splitsmap}[\mathit{bip}]$, adding a key for $\mathit{bip}$ to $\mathit{splitsmap}$ if necessary}\\
    \hspace{3em}$\mathrm{insert}(t, \mathit{splitsmap}[\mathit{bip}])$
    \EndFor
    \EndFor
    \LineComment{Second phase: populate distance matrix}
    \ForAll {$\mathit{treeset} \in \mathrm{values}(\mathit{splitsmap})$} \Comment $\mathrm{values}()$ returns a list of all the values in a dictionary
