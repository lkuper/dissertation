...
In this section, \either{I}{we} highlight a few aspects of the
...
\either{I}{we} discuss a semantic observation about lattices that our
...
lattices, the deltas are essentially just the atoms\lk{I think Aaron
  wrote this.  ``essentially just'' seems weaselly --- what does it
  mean?  Any chance we can get rid of those two words?}---for a set
...
Of course, not every lattice that can be associated with an LVar type
...
\lk{I think everything in this subsection is now up-to-date and
  accurate wrt the current LVish implementation, but it would be great
  if someone else would check it over.  Also waiting for response to
  my 3/14 email.}
...
Listing~\ref{listing-implementation-getLV} gives the implementation
...
\singlespacing
\lstinputlisting[float, caption={Implementation of the \il{getLV}
    function.}, label=listing-implementation-getLV]{chapter4/code/getLV.hs}
\doublespacing
...
avoid imposing extra synchronization on \emph{all} uses of \il{getLV} at
...
\singlespacing
\lstinputlisting[float, caption={Idempotence-assuming implementation
    of the \il{putLV} function.  The version supporting non-idempotent
    (Sched.idemp==False), must also use \il{handlerStatus} as a
    reader-writer lock, except with put being the ``reader'' and
    \il{addHandler} as the writer.},
  label=listing-implementation-putLV]{chapter4/code/putLV.hs}
\doublespacing

\singlespacing
\lstinputlisting[float, caption={Implementation of the \il{freezeLV}
    function.},
  label=listing-implementation-freezeLV]{chapter4/code/freezeLV.hs}
\doublespacing
...
decrement, and checks for equality with zero.\footnote{One can use a
  ...
  implement \il{Counter}.
\rn{Actually, it's implemented and passing tests, so lets turn SNZI on.}}  We use the
...
\lk{This is from the PLDI paper, but shouldn't it also have \il{NoPut}
  as a constraint?}\rn{Why NoPut?  Bumps disallow any optimizations that may
  duplicate work, but puts still work fine, right?}
