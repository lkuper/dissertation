...
\begin{lstlisting}
  data LVar a d = 
       LVar { state :: a,  status :: IORef (Status d),
              handlerStatus :: IORef Handlerstatus }
\end{lstlisting}
\doublespacing
...
\begin{figure}
  ...
  \doublespacing
  \caption{Idempotence-assuming implementation of the \il{putLV}
    function.  The version supporting non-idempotent (Sched.idemp==False), must
    also use \il{handlerStatus} as a reader-writer lock, except with put
    being the ``reader'' and \il{addHandler} as the writer.}\label{fig:implementation-putLV}
\end{figure}
...
Above we saw the additional complexity and cost required to make sure no work is 
duplicated in @getLV@.  Naturally, it is best to pay the aforementioned synchronization
overhead only when required.  We saw that a given instance of the scheduler is
marked with whether the @Par@ monad's effects are idempotent (@Sched.idemp@).
%
Fortunately, LVish's fine-grained effect-tracking capability can provide
precisely this information and thus can determine and set @Sched.idemp@.  We
