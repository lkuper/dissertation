...
Under the lub semantics of @put@, if two independent writes each take
the LVar's contents from, say, $1$ to $2$, then after both writes, its
...
The @put@ operation becomes a family of operations $\PUTi$, with each
corresponding to an update operation $u_i$.  The resulting generalized
language definition is therefore parameterized not only by an
application-specific lattice, but also by an application-specific set
of update operations.  Furthermore, as we will see in
Section~\ref{subsection:quasi-generalized-independence}, we will need
to generalize the Independence lemma
