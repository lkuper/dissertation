...
\lstinputlisting[float, caption={A purely functional Haskell program
    that finds the connected component of the \lstinline|profiles|
    graph that is reachable from the node \lstinline|profile0|, then
    maps the \lstinline|analyze| function over the nodes found.
    Although component discovery proceeds in parallel, we cannot call
    \lstinline|analyze| on any of the nodes in the connected component
