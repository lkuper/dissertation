...
\TODO{Move stuff from \ref{subsection:lvars-inflationary-commutative} to here.}
...
On the other hand, if $\mathit{lv}$'s partial order is that shown in
Figure~\ref{f:lvars-example-lattices}(b), in which the lub of any two
distinct positive integers is $\top$, then \eref{e:lvar-example-1}
...
contents of an LVar, and suppose we change \eref{e:lvar-example-1} to
...
\eref{e:lvar-example-2} is nondeterministic: $x$ might be either $2$
...
We can change \eref{e:lvar-example-2} to behave deterministically by
...
While \eref{e:lvar-example-3} is deterministic, the style of
...
written once, no more than one state from the set $\setof{(\bot, 0),
  (\bot, 1), \dots}$ can ever be reached.  (\either{I}{We} formalize
...
library (which \either{I}{we} will discuss in
\either{Chapter}{Section}~\ref{ch:lvish}) provides unsafe operations
