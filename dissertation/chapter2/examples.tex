...
\TODO{Fix the ridiculous whitespace with these examples.}
...
it updates the LVar to the \emph{least upper bound} (lub) of its
...
and $n_2$ is $\max(n_1, n_2)$.  Therefore \ref{e:lvar-example-1} will
...
distinct positive integers is $\top$, then \ref{e:lvar-example-1} will
...
implementation approach or practical API.  The LVish library (which \either{I}{we}
will discuss in \either{Chapter}{Section}~\ref{ch:lvish}) provides an unsafe @getLV@
