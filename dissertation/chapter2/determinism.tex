...
\emph{local reasoning} about programs that alter state.  Here, $C$ is
a program, and $p$ and $q$ are properties that we assert to be true
before and after $C$ runs, respectively.  Putting them together,
$\lbrace p \rbrace ~ C ~ \lbrace q \rbrace$ is a Hoare triple (in the
style of Hoare logic~\cite{HoareAxiomatic}) specifying the behavior of
$C$.  The frame rule says that, given a program $C$ with precondition
$p$ and postcondition $q$, an assertion $r$ that is \emph{disjoint}
with $p$ and holds before $C$ runs will continue to hold afterward.
