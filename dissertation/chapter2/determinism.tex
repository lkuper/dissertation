...
CnC given by Budimli\'c \etal~\cite{CnC}.  \either{I}{We} reuse the naming
...
\ifdefined\JOURNAL
For most of the supporting lemmas, we only give the statements of the
lemmas here; their proofs appear in~\cite{lvars-dissertation}.
\fi
...
Blelloch~\etal~\cite{blelloch-internally-deterministic}, \either{I}{we} will use
...
\begin{figure}
  ...
  \caption{Comparison of O'Hearn~\etal's frame
    rule~\protect\cite{OHearnLocalReasoning} and a simplified version
...
\either{I}{We} have followed Budimli\'c \etal~\cite{CnC} in treating
