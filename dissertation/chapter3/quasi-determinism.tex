...
status (that is, become frozen) during the transition, $U_S$ acts as
the identity on those locations.  This precondition is only needed in
the {\sc E-Freeze-Final} and {\sc E-Freeze-Simple} cases, and it has
the effect of ruling out interference from freezing.  (Note that $U_S$
need not act as the identity on the contents of locations that are
