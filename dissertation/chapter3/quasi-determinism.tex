...
not.\footnote{To rigorously show that arbitrary updates retain full
  ...
  yet another language, one that generalizes \il{put} to $\PUTi$ but
...
\begin{proof}
  ...
  See Section~\ref{section:clash-proof}.
\end{proof}
...
\begin{proof}
  ...
  Lemma~\ref{lem:locality} (Locality) lemma; at a high level, it shows
  ...
  of the two steps, the proof makes use of the Generalized
  ...
  the $\error$ possibility are those in one step is by {\sc E-Put} and
  the other is by {\sc E-Freeze-Final} or {\sc E-Freeze-Simple}---that
  is, the situations in which a write-after-freeze error is possible.
  See Section~\ref{section:strong-local-quasi-confluence-proof}.
\end{proof}
