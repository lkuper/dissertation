      ...
      \begin{itemize}
        ...
        \item If $l = l'$:
          ...
          \item If $u_{p_i}(u_{p_j}(p_1)) = \topp$:
            ...
            \TODO{symmetric argument for other side.}

            Therefore, by Lemma~\ref{lem:generalized-clash}
            (Generalized Clash), we have that there exists $j' <= 1$
            such that $\config{U_S(S)}{e_{a_1}} \parstepsto^{j'}
            \error$.  Hence
            $\config{\extSRaw{S}{l}{u_{p_j}(p_1)}}{e_{a_1}}
            \parstepsto^{j'} \error$.

            We have that $i' <= 1$ and $j' <= 1$.  If $i' = 0$, we
            would have $\config{\extSRaw{S}{l}{u_{p_i}(p_1)}}{e_{b_1}}
            = \config{S_a}{e_{b_1}} = \error$.  So we would have $S_a
            = \topS$ by the defintion of $\error$, but then we would
            have $\conf_a = \error$, a contradiction.  Therefore $i' =
            1$.

            Similarly, if $j' = 0$, we would have
            $\config{U_S(S)}{e_{a_1}} = \config{S_b}{e_{a_1}} =
            \error$.  So we would have $S_b = \topS$ by the definition
            of $\error$, but then we would have $\conf_b = \error$, a
            contradiction.  Hence $j' = 1$.

            Therefore we have
            $\config{\extSRaw{S}{l}{u_{p_i}(p_1)}}{e_{b_1}}
            \parstepsto \error$ and
            $\config{\extSRaw{S}{l}{u_{p_j}(p_1)}}{e_{a_1}}
            \parstepsto \error$.
    ...
    \item \label{slqc-put-freeze-final}Case {\sc E-Freeze-Final}: We
      ...
      \begin{itemize}
        ...
        \item If $l = l'$:
          ...
          \TODO{This is the one interesting case in the whole
            thing---the conflicting-put-and-freeze case.}
      \end{itemize}
