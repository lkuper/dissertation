...
produce a deterministic result.\footnote{In the LVish library, the
  ...
  computations run; see \either{Chapter}{Section}~\ref{ch:lvish} for
...
\singlespacing
\lstinputlisting[float, caption={A parallel graph traversal in LVish.}, label=listing-bfs_new]{chapter3/code/bfs_new.hs}
\doublespacing

\TODO{As it is, this is not actually running LVish code -- although
  the versions in
  Section~\ref{subsection:lvish-parallel-graph-traversal} are.  We
  could point ahead to that, or we could make this running code, but
  one way or another we either need to acknowledge this.}
